...
| ~~3~~ | ~~**Bloques reutilizables (sincronizados)**~~ **hecho** (ADR-030) | Sección `partialRef` + colección `Partials`: el editor referencia, nunca copia, y editar el partial cambia cada página que lo usa. Al escribir la migración salió un defecto ajeno, ya apuntado en el ADR: `productShowcase` (`dbName: "showcase"`) comparte una sola tabla física entre `pages` y `templates` porque el adaptador de Postgres solo modela un `_parent_id` por nombre de tabla de bloque — sumar `Partials` como tercer aspirante habría reasignado esa FK y roto los `productShowcase` ya publicados. Se excluyó de `Partials.blocks`; la colisión Pages/Templates en sí sigue sin dispararse (nadie ha usado `productShowcase` en una plantilla) y sigue sin arreglo de raíz. |
