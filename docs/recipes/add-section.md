...
1. **`packages/sections/src/blocks/<slug>/index.tsx`** — un único archivo que exporta el resultado de `defineSection({...})` con:
   ...
   - `appearance`: array de `ControlName` de `@courvia/appearance` (hoy: `spaceBlockStart`, `spaceBlockEnd`, `background`, `width`, `align`, `themeScope`). `spaceBlockStart` y `spaceBlockEnd` son obligatorios.
